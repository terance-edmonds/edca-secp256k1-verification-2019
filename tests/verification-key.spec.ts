...
import { EcdsaSecp256k1VerificationKey2019 } from '../src';
...
describe('EcdsaSecp256k1VerificationKey2019', () => {
    ...
    describe('constructor', () => {
        it('sets publicKeyBase58 if not provided but privateKeyBase58 is provided', () => {
        ...
        it('throw if no publicKey58 is provided', () => {
            let error;
            let suite;
            try {
                suite = new EcdsaSecp256k1VerificationKey2019({
                    controller: privateKeyPair.controller,
                    id: privateKeyPair.id
                });
            } catch (e) {
    ...
    describe('generate', () => {
        ...
        it('generates a key pair with compressed publicKey', async () => {
            const key = await EcdsaSecp256k1VerificationKey2019.generate({
                controller: 'did:example:hello',
                id: 'did:example:hello#123',
                compressed: true
        ...
        it('generates a key pair from a seed', async () => {
    ...
    describe('export', () => {
      ...
      `);
        ...
        it('exports with context', () => {
            const key = EcdsaSecp256k1VerificationKey2019.from(privateKeyPair);

            expect(key.export({ privateKey: true, publicKey: true, includeContext: true }))
      ...
      `);
        ...
        describe('throws', () => {
            ...
            it('when trying to export privateKey when constructed from publicKeyPair', () => {
                const key = EcdsaSecp256k1VerificationKey2019.from(publicKeyPair);

                expect(() => key.export({ privateKey: true })).toThrowErrorMatchingInlineSnapshot(
                    '"No privateKey to export."'
                );
            });
        });
    });
    ...
    describe('from', () => {
        it('creates key with exported keyPair', () => {
            ...
            expect(imported.export({ publicKey: true, privateKey: true })).toEqual(exported);
        });

        it('creates key with hex keys', () => {
            const imported = EcdsaSecp256k1VerificationKey2019.from({
                controller: 'did:example:hello',
                id: 'did:example:hello#123',
                privateKeyHex: '16e4438c119c762c2e217470a4ebe119430a3043b4db77604dbafc4dbccb85fd'
            });
